{
  ...
  Flat shaded cube with z light source demo. Drawn with quads.
  ...
  repeat
    ...
    for loop1:=maxpolygons-maxvisible to maxpolygons do
      draw_quad(xp[polygons[pind[loop1],1]],yp[polygons[pind[loop1],1]],
                ...
                xp[polygons[pind[loop1],4]],yp[polygons[pind[loop1],4]],polyz[loop1]);
...
begin
  setmcga;
  setupvirtual;
  for loop1:=1 to 256 do pal(loop1,loop1,loop1,loop1);
  mainloop;
  shutdownvirtual;
  settext;
end.
